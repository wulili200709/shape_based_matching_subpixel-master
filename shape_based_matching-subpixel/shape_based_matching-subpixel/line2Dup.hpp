...
public:
    ...
    bool extractTemplate(Template &templ, const std::vector<cv::Point2f>& pts) const;
...
public:
    ...
    int addTemplate(const cv::Mat sources, const std::string &class_id,
									...
									float angle = 0.0f, float scale = 1.0f, const std::vector<cv::Point2f>& pts = std::vector<cv::Point2f>());
	...
	*/
	cv::Mat getIcpAffine(Match& match, Scene_edge& scene, cuda_icp::RegistrationResult& result, bool& valid, const cv::Rect& roi = cv::Rect());

	cv::Mat getIcpAffineDefect(const cv::Mat image, Match& match, Scene_edge& scene, cuda_icp::RegistrationResult& result, bool& valid, const cv::Rect& roi = cv::Rect());
	...
	int readClasses(const std::string str, const std::vector<std::string> &class_ids = std::vector<std::string>());
	void writeClasses(const std::string str);

	//add by HuangLi 2019/08/14
	void createModel(const cv::Mat& src, cv::Mat& dst, const cv::Mat& mask = cv::Mat(), const cv::Mat& invalidMask = cv::Mat(), std::string modelName = "train", cv::Rect roi = cv::Rect());
	int saveModel(std::string path);
	void detect(cv::Mat &src, cv::Mat& dst, std::vector<FindObject>& findObjects, float matchThreshold, int amount = 10000, const cv::Mat &mask = cv::Mat());
	...
	void detectPose(cv::Mat& src, cv::Mat& dst, std::vector<FindObject>& findObjects, float matchThreshold, int amount = 1, const cv::Mat& mask = cv::Mat());
	void detectPoseAndDefect(cv::Mat& src, cv::Mat& dst, std::vector<FindObject>& findObjects, float matchThreshold, int amount = 1, const cv::Mat &mask = cv::Mat());

	int detectObject(cv::Mat& src, cv::Mat& dst, std::vector<FindObject>& findObjects, float matchThreshold, const std::vector<std::string>& classIds,
																			float startAngle = -360, float endAngle = 360,  int amount = 1, const cv::Mat& mask = cv::Mat());
	//flag 是否归0角度统一计算
	int detectObjectDefect(cv::Mat& src, cv::Mat& dst, line2Dup::Template& templateInfo, float matchThreshold, int amount = 1, const cv::Mat& mask = cv::Mat(), bool flag = 0);
...
public:
    ...
    shapeInfo_producer(const cv::Mat& src, const cv::Mat& mask = cv::Mat(), const cv::Mat& edgeMask = cv::Mat())
	{
        ...
        if(mask.empty())
		{
            ...
            this->mask = cv::Mat(src.size(), CV_8UC1, cv::Scalar(255));
        }
		else
		{
            this->mask = mask;
		...
		if (edgeMask.empty())
		{
			// `addTemplate()` passes this through as the modality mask.
			// When no dedicated edge mask is provided, keep the full object mask enabled.
			this->edgeMask = this->mask.clone();
		}
		else
		{
