...
#pragma omp parallel for
    for(int i=0; i < (int)model_pcd.size(); i++){
...
#else
        // MSVC OpenMP 2.0 does not support declare reduction or custom type reduction;
        // fall back to a serial loop.
        for(int pcd_iter=0; pcd_iter<(int)model_pcd.size(); pcd_iter++){
            reducer += trasnformer(model_pcd[pcd_iter]);
        }
#endif
